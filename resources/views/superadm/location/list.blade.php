@extends('superadm.layout.master')

@section('content')
<div class="row">
  <div class="col-12">
    <div class="card">
      <div class="card-body">

        <div class="mb-3 d-flex justify-content-end">
            <a href="{{ route('locations.create') }}" class="btn btn-add">Add Location</a>
        </div>

        @if (session('success'))
            <div class="alert alert-success alert-dismissible fade show" id="success-alert">{{ session('success') }}</div>
        @endif

        @if (session('error'))
            <div class="alert alert-danger alert-dismissible fade show" id="error-alert" role="alert">
                {{ session('error') }}
            </div>
        @endif

        <div class="table-responsive">
          <table class="table table-bordered table-striped datatables">
            <thead>
              <tr>
                <th>Sr.No.</th>
                <th>State</th>
                <th>District/Area</th>
                <th>City</th>
                <th>Radius</th>
                <th>Type</th>
                <th>Status</th>
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              @foreach($locations as $k => $loc)
                <tr>
                  <td>{{ $k+1 }}</td>
                  <td>{{ $loc->state->state ?? 'N/A' }}</td>
                  <td>{{ $loc->district->district ?? 'N/A' }}</td>
                  <td>{{ $loc->city->city ?? 'N/A' }}</td>
                  <td>{{ $loc->radius }}</td>
                  <td>{{ $types[$loc->type_id] ?? 'N/A' }}</td>
                  <td>
                    <label class="switch">
                      <input type="checkbox" class="toggle-status"
                             data-id="{{ base64_encode($loc->id) }}"
                             {{ $loc->is_active ? 'checked' : '' }}>
                      <span class="slider"></span>
                    </label>
                  </td>
                    <td>
                        <a href="{{ route('locations.edit', base64_encode($loc->id)) }}"
                            class="btn btn-sm btn-primary" title="Edit">
                            <i class="mdi mdi-square-edit-outline icon-medium"></i>
                        </a>

                        <form action="{{ route('locations.delete') }}" method="POST" class="d-inline-block">
                            @csrf
                            <input type="hidden" name="id" value="{{ base64_encode($loc->id) }}">
                            <button type="button" class="btn btn-sm btn-danger delete-btn" title="Delete">
                                <i class="mdi mdi-trash-can-outline icon-medium"></i>
                            </button>
                        </form>
                    </td>
                </tr>
              @endforeach
            </tbody>
          </table>
        </div>

      </div>
    </div>
  </div>
</div>

<script>
$(document).on('change', '.toggle-status', function(){
    let cb = $(this);
    let id = cb.data('id');
    let is_active = cb.is(':checked') ? 1 : 0;

    Swal.fire({
        title: "Are you sure?",
        text: "Do you want to change the status?",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#28a745",
        cancelButtonColor: "#d33",
        confirmButtonText: "Yes, change it!",
    }).then(res => {
        if (res.isConfirmed) {
            $.post("{{ route('locations.updatestatus') }}", {_token: "{{ csrf_token() }}", id: id, is_active: is_active}, function(resp){
                if (!resp.status) {
                    Swal.fire("Error", resp.message, "error");
                    cb.prop('checked', !is_active);
                } else {
                    Swal.fire("Success", resp.message, "success");
                }
            }).fail(function(){ Swal.fire("Error","Something went wrong","error"); cb.prop('checked', !is_active); });
        } else {
            cb.prop('checked', !cb.is(':checked'));
        }
    });
});

$(document).on('click', '.delete-btn', function(){
    let form = $(this).closest('form');
    Swal.fire({
        title: "Confirm",
        text: "This location will be deleted!",
        icon: "warning",
        showCancelButton: true,
        confirmButtonColor: "#28a745",
        cancelButtonColor: "#d33",
        confirmButtonText: "Yes, change it!",
    }).then(res => { if (res.isConfirmed) form.submit(); });
});
</script>

@endsection
