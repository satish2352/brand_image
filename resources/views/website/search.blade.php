...
@section('content')
    ...
    {{-- The bar, the dialog and the sending live in one partial, shared with
         the Map page: both let the team tick hoardings, both hand the same set
         to the same endpoint. All this page adds is how a hoarding is ticked
         here - a checkbox on its card. --}}
    @include('website.includes.share-link')

    @if (site_admin())
        <script>
            (function () {
                const share = window.BrandAddaShare;
                if (!share) return;
                ...
                function setBox(box, on) {
                    box.checked = on;
                    share.toggle(box.value, on);
                    box.closest('.media-card')?.classList.toggle('is-picked', on);
                }

                // Keep the master tick honest: checked only when every card on
                // screen is picked, indeterminate while it is a partial
                // selection, so it never claims more than it has done.
                share.onRender(function () {
                ...
                document.addEventListener('change', function (e) {
                    ...
                    if (box) setBox(box, box.checked);
                });

                // The whole result set renders up front now, so nothing is
                // appended after load. Kept anyway: it costs one observer and
                // keeps the "x of y" hint and the master tick correct for any
                // future code that touches the list.
                const listEl = document.getElementById('media-container');
                if (listEl && window.MutationObserver) {
                    new MutationObserver(() => share.refresh()).observe(listEl, { childList: true });
                }
            })();
        </script>
    @endif
